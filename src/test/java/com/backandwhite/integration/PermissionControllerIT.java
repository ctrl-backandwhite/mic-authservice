...
class PermissionControllerIT extends BaseIntegration {
    ...
    @Autowired private PermissionJpaRepositoryAdapter permissionRepository;
    ...
    @Test void create() {
        ...
        PermissionDtoOut response = webTestClient.post().uri(PATH)
                ...
                .expectStatus().isCreated().expectBody(PermissionDtoOut.class).returnResult().getResponseBody();
    ...
    @Test void findAll() {
        ...
        List<PermissionDtoOut> response = webTestClient.get().uri(PATH)
                ...
                .expectStatus().isOk().expectBodyList(PermissionDtoOut.class).returnResult().getResponseBody();
        assertThat(response).usingRecursiveComparison()
                .ignoringFieldsMatchingRegexes(".*createdAt", ".*updatedAt", ".*createdBy", ".*updatedBy")
    ...
    @Test void getById() {
        ...
        PermissionDtoOut response = webTestClient.get().uri(PATH + "/" + saved.getId())
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME))).exchange()
                .expectStatus().isOk().expectBody(PermissionDtoOut.class).returnResult().getResponseBody();
        assertThat(response).usingRecursiveComparison()
                .ignoringFieldsMatchingRegexes(".*createdAt", ".*updatedAt", ".*createdBy", ".*updatedBy")
    ...
    @Test void update() {
        ...
        PermissionDtoOut response = webTestClient.put().uri(PATH + "/" + saved.getId())
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME)))
                .contentType(MediaType.APPLICATION_JSON).bodyValue(updateDto).exchange()
                .expectStatus().isOk().expectBody(PermissionDtoOut.class).returnResult().getResponseBody();
        assertThat(response).usingRecursiveComparison()
                .ignoringFieldsMatchingRegexes(".*createdAt", ".*updatedAt", ".*createdBy", ".*updatedBy")
                .isEqualTo(permissionDtoOut(saved.getId()));
