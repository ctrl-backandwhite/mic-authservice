...
class UserControllerIT extends BaseIntegration {
        ...
        void create() {
                ...
                UserDtoOut response = webTestClient.post().uri(PATH)
                                ...
                                .isCreated()
                                .expectHeader().contentType(MediaType.APPLICATION_JSON).expectBody(UserDtoOut.class)
                                .returnResult()
                                .getResponseBody();
        ...
        void findAll() {
                ...
                GroupEntity group = groupRepository.save(adminGroupEntity().withId(null).withRoles(List.of(role)));
                GroupEntity otherGroup = groupRepository
                                .save(userGroupEntity().withId(null).withRoles(List.of(otherRole)));

                repository.saveAll(
                                List.of(userEntity().withId(null).withRoles(List.of(role)).withGroups(List.of(group)),
                                                otherUserEntity().withId(null).withRoles(List.of(otherRole))
        ...
        void forgotPassword_whenEmailBlank_returnsBadRequest() {
                ...
                webTestClient.post().uri(PATH + "/forgot-password").contentType(MediaType.APPLICATION_JSON)
                                .bodyValue(dto)
        ...
        void resetPassword_whenInvalidToken_redirectsToError() {
                ResetPasswordDtoIn dto = ResetPasswordDtoIn.builder().token("non-existent-token")
                                .newPassword("NewPass1")
                                .build();

                webTestClient.post().uri(PATH + "/reset-password").contentType(MediaType.APPLICATION_JSON)
                                .bodyValue(dto)
        ...
        class Security {
                ...
                void create_withUserRole_returnsForbidden() {
                        ...
                        webTestClient.post().uri(PATH)
                                        ...
                                        .contentType(MediaType.APPLICATION_JSON).bodyValue(dtoIn).exchange()
                                        .expectStatus().isForbidden();
                }

                @Test
                void create_withoutToken_returnsUnauthorized() {
                        webTestClient.post().uri(PATH).contentType(MediaType.APPLICATION_JSON).bodyValue(userDtoIn())
                                        .exchange()
                ...
                void resetPassword_withoutToken_isPermitted() {
                        ResetPasswordDtoIn dto = ResetPasswordDtoIn.builder().token("any-token").newPassword("NewPass1")
                                        .build();
                        webTestClient.post().uri(PATH + "/reset-password").contentType(MediaType.APPLICATION_JSON)
                                        .bodyValue(dto)
                ...
                void changePasswordRequest_withUserRole_isPermitted() {
                        ...
                        webTestClient.post().uri(PATH + "/change-password/request")
                                        .header(HttpHeaders.AUTHORIZATION,
                                                        getToken(List.of(USER_UNIQUE_NAME), "cpuser@example.com"))
                                        .contentType(MediaType.APPLICATION_JSON).bodyValue(dto).exchange()
                                        .expectStatus().isOk();
                }
                ...
                void sessions_withUserRole_isPermitted() {
                        repository.save(userEntity().withId(null).withNickName("sessuser")
                                        .withEmail("sessuser@example.com")
                                        .withRoles(List.of()).withGroups(List.of()));

                        webTestClient.get().uri(PATH + "/sessions")
                                        .header(HttpHeaders.AUTHORIZATION,
                                                        getToken(List.of(USER_UNIQUE_NAME), "sessuser@example.com"))
                                        .exchange().expectStatus().isOk();
                }
