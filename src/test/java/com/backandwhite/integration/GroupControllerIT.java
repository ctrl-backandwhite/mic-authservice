    ...
    class Security {
        ...
        void create_withUserRole_returnsForbidden() {
            webTestClient.post().uri(PATH).header(HttpHeaders.AUTHORIZATION, getToken(List.of(USER_UNIQUE_NAME)))
                    .contentType(MediaType.APPLICATION_JSON).bodyValue(adminGroupDtoIn()).exchange().expectStatus()
                    .isForbidden();
        }
        ...
        void create_withoutToken_returnsUnauthorized() {
            webTestClient.post().uri(PATH).contentType(MediaType.APPLICATION_JSON).bodyValue(adminGroupDtoIn())
