...
class RedirectUriControllerIT extends BaseIntegration {
    ...
    void create() {
        ...
        RedirectUriDtoOut response = webTestClient.post().uri(PATH)
                ...
                .expectHeader().contentType(MediaType.APPLICATION_JSON).expectBody(RedirectUriDtoOut.class)
                .returnResult().getResponseBody();

        assertThat(response).usingRecursiveComparison()
                .ignoringFieldsMatchingRegexes(".*createdAt", ".*updatedAt", ".*createdBy", ".*updatedBy")
                .ignoringFields("id").isEqualTo(redirectUriDtoOut(null));
    }

    @Test
    void findAll() {
        repository.saveAll(List.of(redirectUriEntity().withId(null), otherRedirectUriEntity().withId(null)));

        List<RedirectUriDtoOut> response = webTestClient.get().uri(PATH)
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME))).exchange().expectStatus()
                .isOk().expectHeader().contentType(MediaType.APPLICATION_JSON).expectBodyList(RedirectUriDtoOut.class)
                .returnResult().getResponseBody();

        assertThat(response).usingRecursiveComparison()
                .ignoringFieldsMatchingRegexes(".*createdAt", ".*updatedAt", ".*createdBy", ".*updatedBy")
                .ignoringFields("id").isEqualTo(List.of(redirectUriDtoOut(null), otherRedirectUriDtoOut(null)));
    }
    ...
    void getById() {
        ...
        RedirectUriDtoOut response = webTestClient.get().uri(PATH + "/" + saved.getId())
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME))).exchange().expectStatus()
                .isOk().expectHeader().contentType(MediaType.APPLICATION_JSON).expectBody(RedirectUriDtoOut.class)
                .returnResult().getResponseBody();
    ...
    void update() {
        ...
        RedirectUriDtoOut response = webTestClient.put().uri(PATH + "/" + saved.getId())
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME)))
    ...
    class Security {
        ...
        void create_withUserRole_returnsForbidden() {
            webTestClient.post().uri(PATH).header(HttpHeaders.AUTHORIZATION, getToken(List.of(USER_UNIQUE_NAME)))
                    .contentType(MediaType.APPLICATION_JSON).bodyValue(redirectUriDtoIn()).exchange().expectStatus()
                    .isForbidden();
        }
        ...
        void create_withoutToken_returnsUnauthorized() {
            webTestClient.post().uri(PATH).contentType(MediaType.APPLICATION_JSON).bodyValue(redirectUriDtoIn())
