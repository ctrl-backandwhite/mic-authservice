...
class OauthClientControllerIT extends BaseIntegration {
    ...
    void create() {
        ...
        OauthClientDtoIn dtoIn = oauthClientDtoIn().withScopeIds(List.of(scope.getId()))
                .withRedirectUriIds(List.of(redirectUri.getId())).withGrantTypeIds(List.of(grantType.getId()));
        ...
        OauthClientDtoOut response = webTestClient.post().uri(PATH)
                ...
                .expectHeader().contentType(MediaType.APPLICATION_JSON).expectBody(OauthClientDtoOut.class)
    ...
    void findAll() {
        ...
        List<OauthClientEntity> saved = repository.saveAll(List.of(
                oauthClientEntity().withId(null).withScopes(List.of(scope)).withRedirectUris(List.of(redirectUri))
                        .withGrantTypes(List.of(grantType)),
                otherOauthClientEntity().withId(null).withScopes(List.of(scope)).withRedirectUris(List.of(redirectUri))
                        .withGrantTypes(List.of(grantType))));
        ...
        List<OauthClientDtoOut> response = webTestClient.get().uri(PATH)
                ...
                .isOk().expectHeader().contentType(MediaType.APPLICATION_JSON).expectBodyList(OauthClientDtoOut.class)
                .returnResult().getResponseBody();
    ...
    void getById() {
        ...
        OauthClientDtoOut response = webTestClient.get().uri(PATH + "/" + saved.getId())
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME))).exchange().expectStatus()
                .isOk().expectHeader().contentType(MediaType.APPLICATION_JSON).expectBody(OauthClientDtoOut.class)
                .returnResult().getResponseBody();
    ...
    class Security {
        ...
        void create_withUserRole_returnsForbidden() {
            webTestClient.post().uri(PATH).header(HttpHeaders.AUTHORIZATION, getToken(List.of(USER_UNIQUE_NAME)))
                    .contentType(MediaType.APPLICATION_JSON).bodyValue(oauthClientDtoIn()).exchange().expectStatus()
                    .isForbidden();
        }
        ...
        void create_withoutToken_returnsUnauthorized() {
            webTestClient.post().uri(PATH).contentType(MediaType.APPLICATION_JSON).bodyValue(oauthClientDtoIn())
