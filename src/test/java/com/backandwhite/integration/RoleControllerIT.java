...
class RoleControllerIT extends BaseIntegration {
    ...
    void create() {
        ...
        RoleDtoOut response = webTestClient.post().uri(PATH_ROLES)
                ...
                .expectHeader().contentType(MediaType.APPLICATION_JSON).expectBody(RoleDtoOut.class).returnResult()
                .getResponseBody();

        assertThat(response).usingRecursiveComparison()
                .ignoringFieldsMatchingRegexes(".*createdAt", ".*updatedAt", ".*createdBy", ".*updatedBy")
                .ignoringFields("id").isEqualTo(adminRoleDtoOut(null));
    }

    @Test
    void findAll() {
        roleRepository.saveAll(List.of(adminRoleEntity().withId(null), userRoleEntity().withId(null)));

        List<RoleDtoOut> response = webTestClient.get().uri(PATH_ROLES)
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME))).exchange().expectStatus()
                .isOk().expectHeader().contentType(MediaType.APPLICATION_JSON).expectBodyList(RoleDtoOut.class)
                .returnResult().getResponseBody();

        assertThat(response).usingRecursiveComparison()
                .ignoringFieldsMatchingRegexes(".*createdAt", ".*updatedAt", ".*createdBy", ".*updatedBy")
                .ignoringFields("id").isEqualTo(List.of(adminRoleDtoOut(null), userRoleDtoOut(null)));
    }

    @Test
    void getById() {
        RoleEntity saved = roleRepository.save(roleEntity().withId(null));

        RoleDtoOut response = webTestClient.get().uri(PATH_ROLES + "/" + saved.getId())
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME))).exchange().expectStatus()
                .isOk().expectHeader().contentType(MediaType.APPLICATION_JSON).expectBody(RoleDtoOut.class)
                .returnResult().getResponseBody();
    ...
    @Test
    void update() {
        RoleEntity saved = roleRepository.save(adminRoleEntity().withId(null));
        RoleDtoIn updateDto = supportRoleDtoIn();

        RoleDtoOut response = webTestClient.put().uri(PATH_ROLES + "/" + saved.getId())
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME)))
    ...
    void delete() {
        ...
        webTestClient.delete().uri(PATH_ROLES + "/" + saved.getId())
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME))).exchange().expectStatus()
                .isNoContent();
