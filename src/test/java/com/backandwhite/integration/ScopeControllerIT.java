...
class ScopeControllerIT extends BaseIntegration {
    ...
    void create() {
        ...
        ScopeDtoOut response = webTestClient.post().uri(PATH)
                ...
                .expectHeader().contentType(MediaType.APPLICATION_JSON).expectBody(ScopeDtoOut.class).returnResult()
                .getResponseBody();
    ...
    void findAll() {
        ...
        List<ScopeDtoOut> response = webTestClient.get().uri(PATH)
                ...
                .isOk().expectHeader().contentType(MediaType.APPLICATION_JSON).expectBodyList(ScopeDtoOut.class)
    ...
    void getById() {
        ...
        ScopeDtoOut response = webTestClient.get().uri(PATH + "/" + saved.getId())
                .header(HttpHeaders.AUTHORIZATION, getToken(List.of(ADMIN_UNIQUE_NAME))).exchange().expectStatus()
                .isOk().expectHeader().contentType(MediaType.APPLICATION_JSON).expectBody(ScopeDtoOut.class)
                .returnResult().getResponseBody();
    ...
    class Security {
        ...
        void create_withUserRole_returnsForbidden() {
            webTestClient.post().uri(PATH).header(HttpHeaders.AUTHORIZATION, getToken(List.of(USER_UNIQUE_NAME)))
                    .contentType(MediaType.APPLICATION_JSON).bodyValue(readScopeDtoIn()).exchange().expectStatus()
                    .isForbidden();
        }
        ...
        void create_withoutToken_returnsUnauthorized() {
            webTestClient.post().uri(PATH).contentType(MediaType.APPLICATION_JSON).bodyValue(readScopeDtoIn())
