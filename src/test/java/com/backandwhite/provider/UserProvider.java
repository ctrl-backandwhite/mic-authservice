...
public final class UserProvider {
    ...
    public static User user() {
        return User.builder().id(USER_ID).name(USER_NAME).lastName(USER_LAST_NAME).nickName(USER_NICK_NAME)
                ...
                .credentialsNonExpired(USER_CREDENTIALS_NON_EXPIRED).roles(List.of(RoleProvider.adminRole()))
                .groups(List.of(GroupProvider.adminGroup())).createdAt(AuditProvider.CREATED_AT)
    ...
    public static User otherUser() {
        return User.builder().id(OTHER_USER_ID).name(OTHER_USER_NAME).lastName(OTHER_USER_LAST_NAME)
                .nickName(OTHER_USER_NICK_NAME).email(OTHER_USER_EMAIL).password(OTHER_USER_PASSWORD)
                .enabled(OTHER_USER_ENABLED).accountNonExpired(OTHER_USER_ACCOUNT_NON_EXPIRED)
                .accountNonLocked(OTHER_USER_ACCOUNT_NON_LOCKED)
                .credentialsNonExpired(OTHER_USER_CREDENTIALS_NON_EXPIRED).roles(List.of(RoleProvider.userRole()))
                .groups(List.of(GroupProvider.userGroup())).createdAt(AuditProvider.CREATED_AT)
                .updatedAt(AuditProvider.UPDATED_AT).createdBy(AuditProvider.CREATED_BY)
                .updatedBy(AuditProvider.UPDATED_BY).build();
    }

    public static UserEntity userEntity() {
        return UserEntity.builder().id(USER_ID).name(USER_NAME).lastName(USER_LAST_NAME).nickName(USER_NICK_NAME)
                .email(USER_EMAIL).password(USER_PASSWORD).enabled(USER_ENABLED)
                .accountNonExpired(USER_ACCOUNT_NON_EXPIRED).accountNonLocked(USER_ACCOUNT_NON_LOCKED)
                .credentialsNonExpired(USER_CREDENTIALS_NON_EXPIRED).roles(List.of(RoleProvider.roleEntity()))
                .groups(List.of(GroupProvider.groupEntity())).createdAt(AuditProvider.CREATED_AT)
                .updatedAt(AuditProvider.UPDATED_AT).createdBy(AuditProvider.CREATED_BY)
                .updatedBy(AuditProvider.UPDATED_BY).build();
    }

    public static UserEntity otherUserEntity() {
        return UserEntity.builder().id(OTHER_USER_ID).name(OTHER_USER_NAME).lastName(OTHER_USER_LAST_NAME)
                .nickName(OTHER_USER_NICK_NAME).email(OTHER_USER_EMAIL).password(OTHER_USER_PASSWORD)
                .enabled(OTHER_USER_ENABLED).accountNonExpired(OTHER_USER_ACCOUNT_NON_EXPIRED)
                .accountNonLocked(OTHER_USER_ACCOUNT_NON_LOCKED)
                .credentialsNonExpired(OTHER_USER_CREDENTIALS_NON_EXPIRED).roles(List.of(RoleProvider.userRoleEntity()))
                .groups(List.of(GroupProvider.userGroupEntity())).createdAt(AuditProvider.CREATED_AT)
                .updatedAt(AuditProvider.UPDATED_AT).createdBy(AuditProvider.CREATED_BY)
                .updatedBy(AuditProvider.UPDATED_BY).build();
    }

    public static UserDtoIn userDtoIn() {
        return UserDtoIn.builder().name(USER_NAME).lastName(USER_LAST_NAME).nickName(USER_NICK_NAME).email(USER_EMAIL)
                .password(USER_PASSWORD).confirmPassword(USER_PASSWORD).enabled(USER_ENABLED)
                .accountNonExpired(USER_ACCOUNT_NON_EXPIRED).accountNonLocked(USER_ACCOUNT_NON_LOCKED)
                .credentialsNonExpired(USER_CREDENTIALS_NON_EXPIRED).roleIds(List.of(RoleProvider.ADMIN_ID))
                .groupIds(List.of(GroupProvider.ADMIN_ID)).build();
    }

    public static UserDtoIn otherUserDtoIn() {
        return UserDtoIn.builder().name(OTHER_USER_NAME).lastName(OTHER_USER_LAST_NAME).nickName(OTHER_USER_NICK_NAME)
                .email(OTHER_USER_EMAIL).password(OTHER_USER_PASSWORD).confirmPassword(OTHER_USER_PASSWORD)
                .enabled(OTHER_USER_ENABLED).accountNonExpired(OTHER_USER_ACCOUNT_NON_EXPIRED)
                .accountNonLocked(OTHER_USER_ACCOUNT_NON_LOCKED)
                .credentialsNonExpired(OTHER_USER_CREDENTIALS_NON_EXPIRED).roleIds(List.of(RoleProvider.USER_ID))
                .groupIds(List.of(GroupProvider.USER_ID)).build();
    }

    public static UserDtoOut userDtoOut(Long id) {
        return UserDtoOut.builder().id(id).name(USER_NAME).lastName(USER_LAST_NAME).nickName(USER_NICK_NAME)
                .email(USER_EMAIL).password(USER_PASSWORD).enabled(USER_ENABLED)
                .accountNonExpired(USER_ACCOUNT_NON_EXPIRED).accountNonLocked(USER_ACCOUNT_NON_LOCKED)
    ...
    public static UserDtoOut otherUserDtoOut(Long id) {
        return UserDtoOut.builder().id(id).name(OTHER_USER_NAME).lastName(OTHER_USER_LAST_NAME)
                .nickName(OTHER_USER_NICK_NAME).email(OTHER_USER_EMAIL).password(OTHER_USER_PASSWORD)
                .enabled(OTHER_USER_ENABLED).accountNonExpired(OTHER_USER_ACCOUNT_NON_EXPIRED)
                .accountNonLocked(OTHER_USER_ACCOUNT_NON_LOCKED)
