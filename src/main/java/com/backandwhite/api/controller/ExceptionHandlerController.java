...
public class ExceptionHandlerController {
    ...
    public ResponseEntity<OperationResponseDtoOut> pSQLExceptionException(DataIntegrityViolationException ex) {
        ...
        return new ResponseEntity<>(OperationResponseDtoOut.builder()
                .code(VALIDATION_ERROR.getCode())
                .message(VALIDATION_ERROR.getDetail())
                .details(buildIntegrityViolationDetails(message))
                .dateTime(ZonedDateTime.now())
                .build(),
    ...
    private List<String> buildIntegrityViolationDetails(String message) {
        if (message == null || message.isBlank()) {
            return List.of("Database constraint violation.");
        }
        ...
        if (normalized.contains("duplicate key value violates unique constraint")) {
            ...
            if (normalized.contains("(nick_name)")
                    || normalized.contains("(user_name)")
                    || normalized.contains("users_nick_name_key")
                    || normalized.contains("users_user_name_key")) {
                return List.of("Username already exists.");
            }
            return List.of("A record with the same unique value already exists.");
        }

        return List.of(message.replaceAll("\\s+", " ").trim());
    }
}
